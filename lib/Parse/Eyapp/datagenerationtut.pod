...
defined by a given context free grammar is simple: I<change the lexer by a generator>. Instead of 
reading from some input, randomly I<generate one of the valid tokens>. 
...
or C<set_tokenweights>. See the code of method C<main> in C<GenSupport.pm>:
  ...
  pl@europa:~/LEyapp/examples/generator$ sed -ne '98,/^ *)/p' GenSupport.pm | cat -n
...
More importantly, the C<Frequency> method produces
one of a given list of elements with a given 
probability distribution.SZee the following example.
...
Now we generate 10 items. We see that odd elements appear more frequently than
even elements:
...
production rule:
  ...
  pl@europa:~/LEyapp/examples/generator$ sed -ne '35,41p' Generator.eyp | cat -n
     1  stmt:
     2      VARDEF '=' exp
     3        {
     4          my $parser = shift;
...
entry in the symbol table:
  ...
  pl@europa:~/LEyapp/examples/generator$ sed -ne '19,24p' GenSupport.pm | cat -n
...
our default action:
  ...
  pl@europa:~/LEyapp/examples/generator$ sed -ne '13,17p' Generator.eyp | cat -n
     1  %defaultaction {
     2    my $parser = shift;
     3
     4    return join '', @_;
...
semicolons:
  ...
  pl@europa:~/LEyapp/examples/generator$ sed -ne '26,33p' Generator.eyp | cat -n
     1  stmts:
     2      stmt
     3        {
...
that the first statment has been processed. In such case we increase the weight
...
=item * How a Script Becomes a Module by Brian d Foy. On Perlmonks: L<http://www.perlmonks.org/index.pl?node_id=396759>.
