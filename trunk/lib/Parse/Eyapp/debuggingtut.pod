...
the C++ grammar, 
  ...
  Parse-Eyapp/examples/debuggingtut$ cat glrexpressions.eyp
...
The grammar illustrates the ambiguity between 
...
(hacking) inside the lexical analizer. If the C<ID> is followed 
by some sequence of closing parenthesis and a C<;> or an C<=> the action 
is to reduce by the production C<declarator -E<gt> ID>:
  ...
  sub _Lexer {
    ...
    for ($input) {
      ...
      if (s{^([a-zA-Z_]\w*)}{}) {
        if (m{^[)\s]*[;=]\s*}) {
          $self->{STATES}[18]{ACTIONS}{')'} = -12
        }
        else {
          $self->{STATES}[18]{ACTIONS}{')'} = -5;
        }
        return ('ID', $1);
      }
...
Here are the results of two executions:
  ...
  lusasoft@LusaSoft:~/src/perl/Parse-Eyapp/examples/debuggingtut$ ./glrexpressions.pm
  int (x) = y+z;
  PROG(EMPTY,DECL(TERMINAL,TERMINAL,PLUS(ID,ID)))
  lusasoft@LusaSoft:~/src/perl/Parse-Eyapp/examples/debuggingtut$ ./glrexpressions.pm
  int (x)+2;
  PROG(EMPTY,EXP(TYPECAST(TERMINAL,ID),NUM))
